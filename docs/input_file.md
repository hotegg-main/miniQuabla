...
|--|--|--|--|--|
|ソルバー    |機体名|name|-||
|           |計算時間刻み|dt|sec|ODEソルバーの時間刻みの設定。基本的に0.001secで良い。|
|           |最大計算時間|t_max|sec|ODEソルバーが計算を行う物理時間の最大値。着地時間を超えている必要があるため、大きめに設定する。|
|           |最小風速|speed_min|m/s|落下分散計算時の最小風速|
|           |風速刻み|speed_step|m/s|落下分散計算時の風速刻み|
|           |風速数|speed_num|-|落下分散計算時の風速数|
|           |最小風向|azimuth_min|deg|落下分散計算時の最小風向|
|           |風向数|azimuth_num|-|落下分散計算時の風向数|
